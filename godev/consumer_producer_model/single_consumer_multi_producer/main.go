...
func producer(pool chan<- string, idx int, wg *sync.WaitGroup) {
	defer wg.Done()
	for _, p := range quotes[idx] {
		fmt.Printf("producer %d send quote '%s'\n", idx, p)
		pool <- p
	}
}

func consumer(pool <-chan string, done chan<- bool) {
	for q := range pool {
		fmt.Printf("consumer consumes quote '%s'\n", q)
	}
	done <- true
}
